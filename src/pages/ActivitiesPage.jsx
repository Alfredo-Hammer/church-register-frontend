...
const CATEGORIES = [
  { value: 'RECOLECCION_FONDOS', label: 'Recolección de Fondos',  color: 'bg-amber-500/20 text-amber-700 dark:text-amber-300' },
  { value: 'SERVICIO_COMUNITARIO', label: 'Servicio Comunitario', color: 'bg-teal-500/20 text-teal-700 dark:text-teal-300' },
  { value: 'CAPACITACION', label: 'Capacitación',                 color: 'bg-blue-500/20 text-blue-700 dark:text-blue-300' },
  { value: 'EVANGELISMO', label: 'Evangelismo',                   color: 'bg-purple-500/20 text-purple-700 dark:text-purple-300' },
  { value: 'SOCIAL', label: 'Social / Convivio',                  color: 'bg-pink-500/20 text-pink-700 dark:text-pink-300' },
  { value: 'OTRO', label: 'Otro',                                 color: 'bg-muted text-muted-foreground' },
];

const STATUSES = [
  { value: 'PLANIFICADA',  label: 'Planificada',  color: 'bg-blue-500/20 text-blue-700 dark:text-blue-300' },
  { value: 'EN_PROGRESO',  label: 'En Progreso',  color: 'bg-yellow-500/20 text-yellow-700 dark:text-yellow-300' },
  { value: 'COMPLETADA',   label: 'Completada',   color: 'bg-green-500/20 text-green-700 dark:text-green-300' },
  { value: 'CANCELADA',    label: 'Cancelada',    color: 'bg-red-500/20 text-red-700 dark:text-red-300' },
];
...
const getCategory = (val) => CATEGORIES.find((c) => c.value === val) || { label: val, color: 'bg-muted text-muted-foreground' };
const getStatus  = (val) => STATUSES.find((s) => s.value === val)   || { label: val, color: 'bg-muted text-muted-foreground' };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Proyectos y actividades especiales de la iglesia</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-card rounded-lg p-4 border border-border">
            <div className="flex items-center gap-2 mb-1">
              <Clock className="h-3.5 w-3.5 text-yellow-700 dark:text-yellow-400" />
              <p className="text-xs text-muted-foreground">En progreso</p>
            </div>
            <p className="text-2xl font-bold text-yellow-700 dark:text-yellow-300">{stats.en_progreso || 0}</p>
          </div>
          <div className="bg-card rounded-lg p-4 border border-border">
            <div className="flex items-center gap-2 mb-1">
              <CheckSquare2 className="h-3.5 w-3.5 text-green-700 dark:text-green-400" />
              <p className="text-xs text-muted-foreground">Completadas</p>
            </div>
            <p className="text-2xl font-bold text-green-700 dark:text-green-300">{stats.completadas || 0}</p>
          </div>
          <div className="bg-card rounded-lg p-4 border border-border">
            <div className="flex items-center gap-2 mb-1">
              <Users className="h-3.5 w-3.5 text-purple-700 dark:text-purple-400" />
              <p className="text-xs text-muted-foreground">Total participantes</p>
            </div>
            <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">{stats.total_participantes || 0}</p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {activities.map((activity) => {
            ...
            return (
              <div key={activity.id} className="bg-card rounded-lg border border-border hover:border-muted-foreground/40 transition-colors flex flex-col">
                {/* Card header */}
                <div className="p-4 flex-1 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {activity.actual_count != null
                            ? <><span className="text-foreground font-medium">{activity.actual_count}</span> participaron</>
                            : null}
                          {activity.actual_count != null && activity.expected_count ? ' / ' : null}
                          {activity.expected_count
                            ? <>{activity.actual_count == null ? '' : ''}<span className="text-muted-foreground">{activity.expected_count} esperados</span></>
                            : null}
                        </span>
                      </div>
                    )}
                  ...
                  {activity.outcome && (
                    <div className="mt-2 p-2 bg-muted/50 rounded text-xs text-muted-foreground italic border-l-2 border-purple-500">
                      {activity.outcome}
                    </div>
                  )}
                </div>
                ...
                {/* Card actions */}
                <div className="border-t border-border px-4 py-2 flex justify-end gap-1">
                  <button
                    onClick={() => openEdit(activity)}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-muted-foreground hover:text-blue-700 dark:text-blue-400 hover:bg-accent rounded transition-colors"
                  >
                    <Pencil className="h-3.5 w-3.5" /> Editar
                  </button>
                  <button
                    onClick={() => setDeleteTarget(activity)}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-muted-foreground hover:text-red-700 dark:text-red-400 hover:bg-accent rounded transition-colors"
                  >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-3 py-1 text-foreground">{page} / {totalPages}</span>
        ...
        <form onSubmit={handleSave}>
          <DialogContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Participantes esperados</label>
                <Input
                  type="number"
                  min="0"
              ...
              {(form.status === 'EN_PROGRESO' || form.status === 'COMPLETADA') && (
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Participantes reales</label>
                  <Input
                    type="number"
                    min="0"
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm placeholder:text-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>

              {/* Notas */}
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-muted-foreground mb-1">Notas / Instrucciones</label>
                <textarea
                  value={form.notes}
              ...
              {isCompleted && (
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Resultado / Conclusión</label>
                  <textarea
                    value={form.outcome}
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)}>
        <DialogHeader onClose={() => setDeleteTarget(null)}>
          <h2 className="text-lg font-bold text-foreground">Eliminar Actividad</h2>
        </DialogHeader>
        <DialogContent>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-foreground font-medium">{deleteTarget?.title}</p>
