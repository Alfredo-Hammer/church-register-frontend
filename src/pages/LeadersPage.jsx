...
import {
  Crown, Plus, Search, Edit, Trash2, X, Phone, Mail,
  Calendar, AlertCircle, CheckCircle, Users, ChevronLeft,
  ChevronRight, MapPin, UserCircle2, Layers,
} from "lucide-react";
import { leadersService, membersService, groupsService } from "@/services/api";
import { useAuth } from "@/contexts/AuthContext";

// ── Constants ─────────────────────────────────────────────────────────────────

const POSITIONS = [
  "Pastor Principal", "Pastor Asociado", "Pastor de Jóvenes",
  "Anciano / Anciana", "Diácono / Diáconisa",
  "Líder de Alabanza", "Líder de Jóvenes", "Líder de Niños",
  "Líder de Células", "Líder de Parejas", "Líder de Damas",
  "Líder de Varones", "Coordinador de Finanzas", "Secretario(a)", "Otro",
];

const AREAS = [
  "Pastoral", "Adoración / Alabanza", "Jóvenes", "Niños",
  "Células / Grupos Pequeños", "Evangelismo", "Misiones",
  "Finanzas", "Administración", "Damas", "Varones", "Parejas", "Otro",
];

const AREA_COLORS = {
  "Pastoral":                 "bg-violet-500/15 text-violet-300 border-violet-500/25",
  "Adoración / Alabanza":     "bg-yellow-500/15 text-yellow-300 border-yellow-500/25",
  "Jóvenes":                  "bg-blue-500/15 text-blue-300 border-blue-500/25",
  "Niños":                    "bg-pink-500/15 text-pink-300 border-pink-500/25",
  "Células / Grupos Pequeños":"bg-teal-500/15 text-teal-300 border-teal-500/25",
...
const GRAD = [
  ...
  "from-emerald-500 to-green-700", "from-amber-500 to-orange-700",
  "from-rose-500 to-pink-700",     "from-cyan-500 to-teal-700",
];
function grad(name = "") {
  let h = 0;
...
function LeaderAvatar({ leader, size = "md" }) {
  const sizes = { sm: "w-9 h-9 text-xs", md: "w-10 h-10 text-sm", lg: "w-20 h-20 text-2xl" };
  const initials = `${leader.firstName?.[0] ?? ""}${leader.lastName?.[0] ?? ""}`.toUpperCase();
  return (
    <div className={`${sizes[size]} rounded-full shrink-0 overflow-hidden bg-gradient-to-br ${grad(leader.firstName + leader.lastName)} flex items-center justify-center font-bold text-white shadow-md`}>
      {leader.photoUrl
        ? <img src={leader.photoUrl} alt={leader.firstName} className="w-full h-full object-cover" />
        : <span>{initials}</span>}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-slate-800 border-l border-slate-700 h-full flex flex-col overflow-hidden shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700 shrink-0">
          <span className="text-sm text-gray-400 font-medium">Perfil del Líder</span>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex flex-col items-center px-5 pt-8 pb-5 border-b border-slate-700">
            ...
            {leader.position && (
              <p className="text-amber-400 font-medium text-sm mt-1">{leader.position}</p>
            )}
            <div className="flex items-center gap-2 mt-2 flex-wrap justify-center">
              {leader.area && (
                <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${areaColor(leader.area)}`}>
                  {leader.area}
                </span>
              )}
          ...
          <div className="px-5 py-4 space-y-3">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Información</h3>
            {leader.groupName && (
              <InfoRow icon={<Users className="w-4 h-4" />} label="Grupo asignado" value={leader.groupName} />
            )}
            {leader.startDate && (
              <InfoRow icon={<Calendar className="w-4 h-4" />} label="En el liderazgo desde" value={fmtDate(leader.startDate)} />
            )}
            <InfoRow icon={<UserCircle2 className="w-4 h-4" />} label="Estado del miembro" value={leader.memberStatus} />
            {leader.notes && (
        ...
        <div className="px-5 py-4 border-t border-slate-700 flex gap-3 shrink-0">
          <Button onClick={() => onEdit(leader)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white">
            <Edit className="w-4 h-4 mr-2" /> Editar
          </Button>
          <Button onClick={() => onDelete(leader)} variant="outline"
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setResults((data.members || []).filter((m) => !excludeIds.includes(m.id)));
      } catch { /* silencioso */ }
    ...
    <div ref={ref} className="relative">
      {value.memberName ? (
        <div className="flex items-center gap-2 px-3 py-2 bg-slate-700 border border-slate-600 rounded-md">
          <div className={`w-7 h-7 rounded-full shrink-0 overflow-hidden bg-gradient-to-br ${grad(value.memberName)} flex items-center justify-center text-xs font-bold text-white`}>
            {value.memberName.split(" ").map((n) => n[0]).slice(0, 2).join("")}
          </div>
          <span className="text-white text-sm flex-1">{value.memberName}</span>
          <button type="button" onClick={() => onChange({ memberId: "", memberName: "" })}
      ...
      {open && !value.memberName && (
        <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-slate-700 border border-slate-600 rounded-lg shadow-xl overflow-hidden">
          ...
          <div className="px-3 py-2 border-b border-slate-600 flex items-center justify-between">
            <span className="text-xs text-gray-500 font-medium">Solo miembros activos</span>
            {loading && <div className="w-3.5 h-3.5 border-2 border-amber-400 border-t-transparent rounded-full animate-spin" />}
          </div>
          ...
          <div className="max-h-52 overflow-y-auto">
            ...
            {results.map((m) => (
              <button key={m.id} type="button" onClick={() => select(m)}
                ...
                <div className={`w-8 h-8 rounded-full shrink-0 overflow-hidden bg-gradient-to-br ${grad(m.first_name + m.last_name)} flex items-center justify-center text-xs font-bold text-white`}>
                  {m.photo_url
                    ? <img src={m.photo_url} alt="" className="w-full h-full object-cover" />
                    : `${m.first_name[0]}${m.last_name[0]}`}
                </div>
                <div className="min-w-0">
                  <p className="text-sm text-white font-medium truncate">{m.first_name} {m.last_name}</p>
...
function LeaderModal({ open, onClose, editing, existingIds, onSaved, customPositions, customAreas, onAddPosition, onAddArea }) {
  ...
  const allPositions = [...new Set([...POSITIONS.filter((p) => p !== "Otro"), ...customPositions])];
  const allAreas     = [...new Set([...AREAS.filter((a) => a !== "Otro"), ...customAreas])];

  useEffect(() => {
    groupsService.getAll({ limit: 200 }).then((d) => setGroups(d.groups || [])).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { onSaved(result); onClose(); }, 900);
    } catch (err) {
      setError(err.response?.data?.error || "Error al guardar.");
    } finally { setSaving(false); }
  };

  const fieldCls  = "bg-slate-700 border-slate-600 text-white placeholder-gray-500 focus:border-violet-500";
  const selectCls = "flex-1 px-3 py-2 rounded-md bg-slate-700 border border-slate-600 text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500";
  ...
  return (
    <>
        ...
        <DialogContent className="bg-slate-800 border-slate-700 max-w-lg w-full max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-300">Miembro *</label>
              {editing ? (
                <div className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-md text-gray-400 text-sm">
                  {form.memberName} <span className="text-gray-600">(no se puede cambiar)</span>
                </div>
              ) : (
                <MemberPicker
            ...
            {/* Position */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-300">Posición / Cargo</label>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setAddPos(true)}
            ...
            {/* Area */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-300">Área / Ministerio</label>
              <div className="flex gap-2">
                <select value={form.area} onChange={(e) => set("area", e.target.value)} className={selectCls}>
                  <option value="">Sin especificar</option>
                  {allAreas.map((a) => <option key={a} value={a}>{a}</option>)}
                </select>
                <button type="button" onClick={() => setAddArea(true)}
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Group */}
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-gray-300">Grupo asignado</label>
            ...
            {/* Start date */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-300">Fecha de inicio en el liderazgo</label>
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={form.notes} onChange={(e) => set("notes", e.target.value)}
                rows={3} placeholder="Notas o descripción del ministerio..."
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={saving || !!success}
                ...
                {saving ? "Guardando..." : editing ? "Guardar Cambios" : "Registrar Líder"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <AddOptionDialog
        ...
        label="Cargo"
        onAdd={(v) => { onAddPosition(v); set("position", v); }}
      />
      <AddOptionDialog
        open={addArea}
        onClose={() => setAddArea(false)}
        label="Área"
        onAdd={(v) => { onAddArea(v); set("area", v); }}
      />
    </>
  );
}
...
export default function LeadersPage() {
  ...
  useEffect(() => { load(); }, [filterStatus, filterArea]);
  ...
  useEffect(() => {
    ...
    const fromPos  = leaders.map((l) => l.position).filter((p) => p && !BASE_POS.includes(p));
    const fromArea = leaders.map((l) => l.area).filter((a) => a && !BASE_AREA.includes(a));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Gestiona el equipo de liderazgo de tu iglesia</p>
        </div>
        {canEdit && (
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total Líderes", value: stats.total,   color: "text-white",        bg: "bg-amber-500/10 border-amber-500/20" },
            { label: "Activos",       value: stats.activos, color: "text-emerald-400",  bg: "bg-emerald-500/10 border-emerald-500/20" },
            { label: "Áreas activas", value: stats.areas,   color: "text-blue-400",     bg: "bg-blue-500/10 border-blue-500/20" },
          ].map((s) => (
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <Input placeholder="Buscar por nombre o posición..."
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-700">
          <span className="text-sm font-semibold text-white">
            Equipo de Liderazgo
            <span className="ml-2 text-gray-400 font-normal">({leaders.length})</span>
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <p className="text-sm mt-1">Registra el equipo de liderazgo de tu iglesia</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700 bg-slate-800/60">
                  <th className="text-left py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider">Líder</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider hidden sm:table-cell">Posición</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider hidden md:table-cell">Área</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider hidden lg:table-cell">Contacto</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider hidden lg:table-cell">Grupo</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider">Estado</th>
                  <th className="text-right py-3 px-4 text-gray-400 font-medium text-xs uppercase tracking-wider">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-700/50">
                {leaders.map((l) => (
                  <tr key={l.id} className="hover:bg-slate-700/40 transition-colors cursor-pointer"
                    onClick={() => setDetail(l)}>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white font-medium text-sm">{l.firstName} {l.lastName}</p>
                          {l.startDate && (
                            <p className="text-xs text-gray-500 mt-0.5">Desde {fmtDate(l.startDate)}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4 hidden sm:table-cell">
                      <span className="text-sm text-gray-200">{l.position || "—"}</span>
                    </td>

                    {/* Area */}
                    <td className="py-3 px-4 hidden md:table-cell">
                      {l.area
                        ? <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${areaColor(l.area)}`}>{l.area}</span>
                    ...
                    <td className="py-3 px-4 hidden lg:table-cell" onClick={(e) => e.stopPropagation()}>
                      <div className="space-y-1">
                        ...
                        {l.email && <p className="text-xs text-gray-400 truncate max-w-[160px]">{l.email}</p>}
                        {!l.phone && !l.email && <span className="text-gray-600 text-xs">—</span>}
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4 hidden lg:table-cell">
                      {l.groupName
                        ? <span className="text-xs text-blue-300 flex items-center gap-1"><Users className="w-3 h-3" />{l.groupName}</span>
                        : <span className="text-gray-600 text-sm">—</span>}
                    </td>
      ...
      {detail && (
        <DetailPanel
          ...
          onDelete={(l) => { setDetail(null); setDeleteTarget(l); }}
        />
      )}
      ...
      <LeaderModal
        ...
        onAddPosition={(v) => setCustomPositions((p) => [...new Set([...p, v])])}
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent className="bg-slate-800 border-slate-700 max-w-sm w-full">
          ...
          <p className="mt-3 text-gray-300 text-sm">
            ...
            </span>{" "}del equipo de liderazgo?
          </p>
          <div className="flex justify-end gap-3 mt-6">
            <Button variant="outline" onClick={() => setDeleteTarget(null)}
              className="border-slate-600 text-gray-300 hover:text-white">Cancelar</Button>
