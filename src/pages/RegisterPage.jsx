    ...
    <div className="flex items-center justify-center gap-0 mb-8">
      {steps.map((s, i) => (
        <React.Fragment key={s.n}>
          ...
          {i < steps.length - 1 && (
            <div className={`w-20 h-0.5 mb-5 mx-1 transition-all duration-300 ${step > 1 ? "bg-emerald-500" : "bg-background"}`} />
          )}
        </React.Fragment>
      ))}
    </div>
      ...
      <div
        className={`relative w-20 h-20 rounded-xl border-2 border-dashed cursor-pointer overflow-hidden transition-all duration-200 group
          ${drag
            ...
            : logo
            ? "border-border bg-muted/50 hover:border-blue-500"
            : "border-border bg-muted/50 hover:border-blue-500 hover:bg-blue-500/5"}`}
      ...
      >
        {logo ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-1">
            <Camera className="w-6 h-6 text-muted-foreground group-hover:text-blue-700 dark:text-blue-400 transition-colors" />
            <span className="text-[10px] text-muted-foreground group-hover:text-blue-700 dark:text-blue-400 transition-colors font-medium leading-tight text-center px-1">
              Logo
            </span>
          </div>
        )}
...
function IconInput({ icon: Icon, ...props }) {
  return (
    <div className="relative">
      ...
      <Input {...props} className={`pl-9 bg-muted/50 border-border text-foreground placeholder:text-muted-foreground focus:border-blue-500 ${props.className || ""}`} />
    </div>
  );
}
    ...
    return (
      <div className="surface-public min-h-screen bg-gradient-to-br from-background via-muted to-background flex items-center justify-center p-4">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-3xl font-bold text-foreground mb-3">¡Bienvenido!</h2>
          <p className="text-muted-foreground mb-2 font-medium">{churchName}</p>
  ...
  return (
    <div className="surface-public min-h-screen bg-gradient-to-br from-background via-muted to-background py-8 px-4">
      <div className="relative w-full max-w-2xl mx-auto">
        ...
        {/* Card */}
        <div className="bg-muted/50 backdrop-blur-sm border border-border rounded-2xl shadow-2xl overflow-hidden">
          {/* Error banner */}
          {error && (
            <div className="flex items-start gap-3 px-6 pt-5 pb-0">
              <div className="flex items-start gap-3 w-full bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-300 rounded-xl px-4 py-3">
          ...
          {step === 1 && (
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-3 pb-3 border-b border-border">
                ...
                <div>
                  <h2 className="text-foreground font-semibold">Información de la Iglesia</h2>
                  ...
                  <Field label="Denominación">
                    <div className="relative">
                      ...
                      <select
                        ...
                        className="w-full pl-9 pr-3 py-2 bg-muted/50 border border-border text-foreground rounded-md text-sm focus:outline-none focus:border-blue-500 appearance-none"
                      >
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="flex items-center gap-3 pb-3 border-b border-border">
                ...
                <div>
                  <h2 className="text-foreground font-semibold">Cuenta del Administrador</h2>
                  <p className="text-muted-foreground text-xs">Estas credenciales serán para acceder al sistema</p>
                </div>
              </div>
              ...
              <div className="bg-muted/50 border border-border rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-foreground font-semibold text-sm truncate">{churchName}</p>
                  <p className="text-muted-foreground text-xs">{denomination || "Sin denominación especificada"}{churchCity ? ` · ${churchCity}` : ""}</p>
                </div>
                <button type="button" onClick={() => { setStep(1); setError(""); }}
                  className="ml-auto text-blue-700 dark:text-blue-400 hover:text-blue-700 dark:text-blue-300 text-xs font-medium whitespace-nowrap flex items-center gap-1">
              ...
              <Field label="Contraseña" required>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-9 pr-10 bg-muted/50 border-border text-foreground placeholder:text-muted-foreground focus:border-blue-500"
                  />
              ...
              <Field label="Confirmar Contraseña" required>
                ...
                {confirmPassword && confirmPassword !== password && (
                  <p className="text-xs text-red-700 dark:text-red-400 mt-1">Las contraseñas no coinciden</p>
                )}
              </Field>
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => { setStep(1); setError(""); }}
                  className="flex-1 h-11 bg-background hover:bg-accent text-foreground font-medium rounded-xl flex items-center justify-center gap-2 transition-colors">
  ...
  return (
    <div className="mt-2 space-y-1.5">
      <div className="flex gap-1">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className={`h-1.5 flex-1 rounded-full transition-all ${i < score ? colors[score - 1] : "bg-muted"}`} />
        ))}
      </div>
      <p className={`text-xs font-medium ${score < 2 ? "text-red-700 dark:text-red-400" : score < 3 ? "text-yellow-700 dark:text-yellow-400" : "text-emerald-700 dark:text-emerald-400"}`}>
